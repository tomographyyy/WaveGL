 ...
 * 
 * Modified by Tomohiro Takagawa in 2025
 */

var helperFunctions = '\
  const float IOR_AIR = 1.0;\
  const float IOR_WATER = 1.333;\
  const vec3 abovewaterColor = vec3(0.25+0.55, 1.0-0.1, 1.25-0.25);\
  const vec3 underwaterColor = vec3(0.4+.4, 0.9-0.0, 1.0);\
  const float poolHeight = 1.2;\
  uniform vec3 light;\
...
function Renderer() {
  ...
  this.waterMesh.transform(GL.Matrix.scale(1, 1, 1)); // change 250507
  this.waterShaders = [];
  for (var i = 0; i < 2; i++) {
    ...
    ', helperFunctions + '\
      ...
      vec3 getSurfaceRayColor(vec3 origin, vec3 ray, vec3 waterColor) {\
        ...
        if (q < 1.0e6) {\
          ...
          /*ここで水面から透けて見える底面を反映させている*/\
          /*vec2 t = intersectCube(origin, ray, vec3(-1.0, -poolHeight+0.3, -1.0), vec3(1.0, 2.0, 1.0));*/\
          vec2 t = intersectCube(origin, ray, vec3(-1.0, -poolHeight+0.3, -1.0), vec3(1.0, 2.0, 1.0));\
          color = getWallColor(origin + ray * t.y) * 0.99;\
        } else {\
          vec2 t = intersectCube(origin, ray, vec3(-1.0, -poolHeight, -1.0), vec3(1.0, 2.0, 1.0));\
          vec3 hit = origin + ray * t.y;\
          if (hit.y < 2.0 / 12.0) {\
            color = getWallColor(hit)*2.;\
          } else {\
  ...
  ', helperFunctions + '\
    varying vec3 position;\
    void main() {\
      gl_FragColor = vec4(getSphereColor(position), 1.0) *1.0; /*球のもともとの色*/\
      vec4 info = texture2D(water, position.xz * 0.5 + 0.5);\
      if (position.y < info.r) {\
        gl_FragColor.rgb *= underwaterColor * 1.0;/*水面下で側方から直接見える場合の色*/\
      }\
    }\
  ');
  ...
  this.cubeMesh.transform(GL.Matrix.scale(1, 1, 1));
  ...
  ');
  this.cubeShaderB = new GL.Shader(helperFunctions + '\
    varying vec3 position;\
    void main() {\
      position = gl_Vertex.xyz;\
      position.x = position.x - 1.0;\
      position.y = ((1.0 - position.y) * (7.0 / 12.0) - 1.0+0.9) * poolHeight;\
      gl_Position = gl_ModelViewProjectionMatrix * vec4(position, 1.0);\
    }\
  ', helperFunctions + '\
    varying vec3 position;\
    void main() {\
      ...
      vec4 info = texture2D(water, position.xz * 0.5 + 0.5);\
      if (position.y < info.r) {\
        gl_FragColor.rgb *= underwaterColor * 0.3;\
      }\
    }\
  ');//*/
...
//*
Renderer.prototype.renderCubeB = function() {
  gl.enable(gl.CULL_FACE);
  water.textureA.bind(0);
  this.tileTexture.bind(1);
  this.causticTex.bind(2);
  this.cubeShaderB.uniforms({
    light: this.lightDir,
    water: 0,
    tiles: 1,
    causticTex: 2,
