...
window.onload = function() {
  ...
  document.onkeydown = function(e) {
    ...
    else if (e.which == 'A'.charCodeAt(0)) useSphereAuto = !useSphereAuto;
    else if (e.which == 'S'.charCodeAt(0)) {
      if (water.SHALLOW == 0.0) {
        water.SHALLOW=5.0;
      } else {
        water.SHALLOW=0.0;
      }
  ...
  function update(seconds) {
    ...
    } else if (useSphereAuto) {
      // Fall down with viscosity under water
      var ay = - 0.4 * center.y
      velocity.y += ay * seconds  
      //var percentUnderWater = Math.max(0, Math.min(1, (radius - center.y) / (2 * radius)));
      //velocity = velocity.add(gravity.multiply(seconds - 1.1 * seconds * percentUnderWater));
      //avelocity = velocity.subtract(velocity.unit().multiply(percentUnderWater * seconds * velocity.dot(velocity)));
      center = center.add(velocity.multiply(seconds));
